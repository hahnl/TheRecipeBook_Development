...
$sql = "SELECT R.id, R.name, M.type, R.cooking_time, R.main_ingredient_1, R.main_ingredient_2, R.recipe, (SELECT favorite FROM favs WHERE username = '$username' && recipe_id = R.id), R.username, DATE_FORMAT(R.date, '%d-%M-%Y'), R.url, (SELECT coalesce(ROUND(AVG(RT.rating)), 0) FROM rating RT WHERE RT.recipe_id = R.id), R.desc FROM recipes R INNER JOIN meal_type M ON R.meal_type = M.mid ORDER BY R.name ASC LIMIT $start_from, $num_rec_per_page";
  ...
  <section class="col-md-8">
    ...
    <?php
       ...
       if ($filter != 'All Recipes') {
           ...
           $filtering = "SELECT R.id, R.name, M.type, R.cooking_time, R.main_ingredient_1, R.main_ingredient_2, R.recipe, (SELECT F.favorite FROM F.favs INNER JOIN recipes R WHERE F.recipe_id = R.id AND F.username = '".$username."'), R.username, DATE_FORMAT(R.date, '%d-%M-%Y'), R.url, (SELECT coalesce(ROUND(AVG(RT.rating)), 0) FROM rating RT WHERE RT.recipe_id = R.id), R.desc FROM recipes R INNER JOIN meal_type M ON R.meal_type = M.mid ORDER BY R.name ASC";
       }
    ...
    <article class="blog-item">
      ...
      if ($row[7] === '1') {
        echo "<span class=\"btn-clipboard\">Favorite</span>";
      }
      ?></div></div>
      <div class="row">
        <div class="col-md-3">
...
<?php
$sql = "SELECT R.id, R.name, M.type, R.cooking_time, R.main_ingredient_1, R.main_ingredient_2, R.recipe, R.favorite, R.username, DATE_FORMAT(R.date, '%d-%M-%Y'), R.url, (SELECT coalesce(ROUND(AVG(RT.rating)), 0) FROM rating RT WHERE RT.recipe_id = R.id), R.desc FROM recipes R INNER JOIN meal_type M ON R.meal_type = M.mid ORDER BY R.name ASC";
      ...
      <nav>
        <ul class="pagination pagination-sm">
          ...
          <?php }
            ...
            <li><a href="main.php?page=<?php echo $previous_page; ?>">Previous</a></li>
          <?php }
         ...
         <?php } else { ?>
           <li><a href="main.php?page=<?php echo $next_page; ?>" aria-label="Next"><span aria-hidden="true">Next</span></a></li>
         <?php } ?>
        </ul>
      </nav>
      </div>
