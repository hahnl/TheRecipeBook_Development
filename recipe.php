...
<div class="row">
    ...
    <?php
       ...
       $filtering = "SELECT R.id, R.name, M.type, R.cooking_time, R.main_ingredient_1, R.main_ingredient_2, R.recipe, (SELECT favorite FROM favs WHERE username = '".$username."' && recipe_id = R.id), R.username, DATE_FORMAT(R.date, '%d-%M-%Y'), R.url, (SELECT coalesce(ROUND(AVG(RT.rating)), 0) FROM rating RT WHERE RT.recipe_id = R.id), R.desc FROM recipes R INNER JOIN meal_type M ON R.meal_type = M.mid WHERE R.id = '".$recipe_id."' ORDER BY R.name ASC";
    ...
    <article class="recipe-blog-item">
      <div class="row">
        ...
        <div class="col-md-9">
          ...
          <p> Avg. Rating: <?php echo $row[11]; ?> Stars - Cooking Time: <?php echo $row[3]; ?> minutes <br>
